...
$data.each do |path|
  ...
  x_move = path.point1.x < path.point2.x ? -> { x += 1 } : -> { x -= 1 }
  y_move = path.point1.y < path.point2.y ? -> { y += 1 } : -> { y -= 1 }
  while x != path.point2.x && y != path.point2.y
    $board[[x, y]] = Point.new(x, y, 0) if $board[[x, y]].nil?
    $board[[x, y]].score += 1
    x_move.call
    y_move.call
    $board[[x, y]] = Point.new(x, y, 0) if x == path.point2.x && y == path.point2.y && $board[[x, y]].nil?
    $board[[x, y]].score += 1 if x == path.point2.x && y == path.point2.y
  end
end
